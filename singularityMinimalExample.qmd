...
needs to be installed to the analysis container. Snakemake depends on the
...
installing Snakemake were found
...
A workflow should be organized per [this
template](https://github.com/snakemake-workflows/snakemake-workflow-template).

### TODO: Does this correctly install in the environments? --> copy revised version & check ENVs
...
export PATH="/opt/conda/bin:/opt/conda/envs/snakemake/bin:$PATH"
...
install_packages bzip2 wget ca-certificates

# Install Miniconda from web source
cd /tmp & \
wget $MINICONDA_REPO$MINICONDA_INSTALLER && \
bash $MINICONDA_INSTALLER  -b -p /opt/conda && \
rm $MINICONDA_INSTALLER  

# Install Mamba and create 'base' & 'snakemake' environment
conda install -n base -c conda-forge mamba && \
. $CONDA_DIR/bin/activate base  && \ # 'source' doesnt work, use '.' instead
mamba create -c bioconda -c conda-forge -n snakemake snakemake-minimal && \
mamba clean -a -y && \
. $CONDA_DIR/bin/activate snakemake
...
%help
This is a minimal example container with snakemake.
```

The resulting image is about 281MB is size (check with `du -h debian-snakemake.sif`). The image still has some issues with launching the snakemake environment, but this serves as an example of how to install Snakemake in a singularity container. 

### Snakemake host installation

Conversely to installing Snakemake in each container, it can be installed on the host machine
