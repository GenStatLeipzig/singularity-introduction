...
We will use `ReadTheDocs`, powered by the python documentation generator `Sphinx` using the `sphinx_rtd_theme` to create and host documentation, because it is a widely-used and convenient. Text is written in the `ReStructuredText` (`RST`, files end on `.rst`) markup language

* Using `ReadTheDocs`, based on `Sphinx`
* Reicht Sphinx? (+ einem template)
...
```{bash}
# Go to home directory (or any directory where to place a new project)
cd ~/
