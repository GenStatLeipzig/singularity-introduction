...
The directory mounts need to be specified as environment variable. The directory
containing the workflow will be mounted at `/mnt/calling` and the base genstat
directory at `/mnt/imise`.
...
# check the DAG of the workflow (create an image)
snakemake --configfile config/config.yaml --dag | dot -Tsvg > calling_dag.svg
...
## Documentation with Sphinx & ReadTheDocs TODO

Each analysis container

We will use `ReadTheDocs`, powered by the python documentation generator
`Sphinx`, using the `sphinx_rtd_theme` to create and host documentation,
because it is  widely-used and convenient. Text is written in the
`ReStructuredText` (`RST`, files end on `.rst`) markup language or in regular
Markdown using the [MyST parser extension](https://myst-parser.readthedocs.io/en/latest/intro.html). 
Sphinx can be installed using `conda`/`mamba` and a project can be initialized with
`Sphinx` itself.

```{bash}
# Go to your home directory (or any directory where to place a new project)
cd ~/

# Create a new environment with the sphinx documentation generator
mamba create -c conda-forge -n sphinx sphinx sphinx_rtd_theme myst_parser
conda activate sphinx

# Create a new directory and initialize a new Sphinx project 
# Answer `y` to the first question and enter the corresponding information 
# to the following questions
sphinx-quickstart mySphinxProject
cd mySphinxProject

# check the structure
tree

# render documentation in the form `sphinx-build -b html sourcedir builddir`
...
The configuration file of the project is called `conf.py` and is used to change
the html theme, meta-data, add extensions and other general configuration. For
instance, we can change the default theme `alabaster` to the ReadTheDocs theme
`sphinx_rtd_theme` to render HTML that looks like documentation hosted on
ReadTheDocs.

```{bash}
# Replace the default html theme
sed -i /alabaster/sphinx_rtd_theme/ source/conf.py

# Re-rendering the project will produce different HTML output
make html

# Check the output and notice the different layout
firefox _build/index.html
```

The basics of writing documentation using `ReStructuredText` are explained in
the `Sphinx Quickstart Guide`
[here](https://www.sphinx-doc.org/en/master/usage/quickstart.html). Documentation can then be written in the `RST`-format.

To host the documentation using `ReadTheDocs`

```{bash}
# Green 'Use This Template' button 
https://github.com/readthedocs/tutorial-template

# This is the repository based on the template project above
https://github.com/GenStatLeipzig/nako-documentation
```

* Myst statt RST nutzen? --> Vorteil einfaches MD
* RTD Tutorial: https://docs.readthedocs.io/en/stable/tutorial/index.html
  - Mit GitHub Template & kostenlosem hosting auf RTD --> komplett integriert
* Doc of the RTD sphinx theme: https://sphinx-rtd-tutorial.readthedocs.io/en/latest/folders.html
* sphinx doc:
* https://github.com/readthedocs/tutorial-template/generate to start a repository using the RTD template
* rstcheck pygments m2r2? was machen die?
  + pygments macht syntax highlighting
* extensions: myst_parser (um direkt md zu parsen)
* MkDocs: https://docs.readthedocs.io/en/stable/intro/getting-started-with-mkdocs.html? macht das gleiche wie sphinx, eher nicht?
...
A program similar to Sphinx but simpler to use is `MKDocs`

```{bash}
# Install mkdocs (into the `sphinx` environment) 
mamba install -c conda-forge -n sphinx mkdocs

# neues projekt
mkdocs new NAKO-Calling-Workflow
mv NAKO-Calling-Workflow/* ./
rmdir NAKO-Calling-Workflow/

# This creates the documentation HTML and opens it in a webrowser
mkdocs serve

# In case it doesn't open automatically, start firefox manually
...
### Leipzig HPC Cluster 'Galaxy'

```{bash}
# infrastruktur beantragen --> Uni Login
https://service.rz.uni-leipzig.de/scientific-computing/sc-infrastruktur-beantragen/

# Login konfigurieren (ssh key etc)
https://portal.sc.uni-leipzig.de/account-overview

# login via (replace cw897rwyu with your URZ user name)
# galaxy: ssh cw897rwyu@galaxy101.sc.uni-leipzig.de
ssh cw897rwyu@login01.sc.uni-leipzig.de # has the singularity installation
...
https://www.sc.uni-leipzig.de/user-doc/

# transfer files to the cluster!
https://www.sc.uni-leipzig.de/user-doc/quickstart/hpc/#copy-data-from-and-to-the-sc-systems
```

MPI implementation on clara

```{bash}
# To use MPI load the module, MPICH is also available
module keyword OpenMPI # search for MPI modules
module spider OpenMPI/4.1.4-GCC-11.3.0 # Info about module
...
scp SOME_FILE cw897rwyu@login01.sc.uni-leipzig.de:/home/sc.uni-leipzig.de/cw897rwyu/
scp -r SOME_FOLDER/ cw897rwyu@login01.sc.uni-leipzig.de:/home/sc.uni-leipzig.de/cw897rwyu/
```
...
https://www.sc.uni-leipzig.de/user-doc/tutorials/singularity/
singularity --version
```
...
Testing can be implemented on the level of each script, using test features provided by each programming language, on the level of the workflow language Snakemake and the level of the Singularity container. 

* section `%test`

### Benchmarking TODO

Benchmarking is supported natively by Snakemake.
