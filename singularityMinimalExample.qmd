...
We want to add analysis scripts as R-scripts or snakemake files to containers to use them as isolated analysis modules. Therefore, in addition to programs and libraries necessary for the analysis, the analysis scripts need to be saved in the container. Files to be added to an image can be specified in the definition file in the `$files` section. The details are documented [here](https://docs.sylabs.io/guides/latest/user-guide/definition_files.html#files). For complicated analyses, the input needs to be highly standardized and tested and output needs to be saved in a directory thats accessible for the user but should still be read-only.

*   mount read-only input file directories
*   create output directory and save something
...
### A first minimum example analysis pipeline

*   a container with R and some packages
*   tests for integrity of input data etc...
*   an R script reading some files from some directory (how do i supply different inputs? in the def, in the run command, via mounting /data/data1, /data/data2 etc with identical substructure?)
*   write results into folder of project using variable $USER + some gibberish (for identical name use-cases), e.g. `cfbeuchel_2j3443/resultfile.csv`
