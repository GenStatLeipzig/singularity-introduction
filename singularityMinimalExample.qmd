...
snakemake --draft-notebook results/plots/quals.svg --cores 4 --use-conda --conda-frontend mamba

# for report

# max' workflow nutzen
https://github.com/GenStatLeipzig/NAKO_CallingWorkflow
```
...
Using the container `debian_snakemake.sif` containing a Snakemake installation built above, it is possible to define a workflow, nest it into a container as a complete containerized analysis module. 
...
* using `exec "$@"` as runscript just executes everything given to container
* --> execute shellscript from container with ./container.sif script.sh
* `. /opt/conda/etc/profile.d/conda.sh` allows directly calling `conda activate`
* $ conda env create --name snakemake-tutorial --file environment.yaml
* use template: https://github.com/snakemake-workflows/snakemake-workflow-template/
* `snakemake --use-conda -n -r results/mapped/A.bam` to perform a dry-run and `snakemake --use-conda results/mapped/A.bam --cores 1`
* install dependencies from file


### A genotype calling workflow utilizing Singularity and Snakemake

* add powertools to container!
